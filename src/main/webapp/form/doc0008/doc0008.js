...
********************************************** */
function subDelete() {
	var url = "http://localhost:8080/ibiDoc/DeleteDocumentInfoServlet?ACTION=";
	var action = "delete";
	...
	if (retCode == "1") {
    displayMessage(getMsg("msg0005_008"));  // 削除完了メッセージ
} else if (retCode == "-1") {
    displayMessage(getMsg("msg0006_001"));  // 対象文書なし
} else {
    displayMessage(getMsg("msg0005_009"));  // 削除失敗メッセージ
}
}    
